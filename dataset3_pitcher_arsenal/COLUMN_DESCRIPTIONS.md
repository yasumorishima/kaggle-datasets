# Column Descriptions for Dataset 3
...
## Pitch Type Metrics (Major Pitch Types)
...
- All `usage_pct` columns sum to approximately 100% for each pitcher-season row
- `avg_speed` is measured at release point
- `avg_spin` is measured in revolutions per minute (rpm)
- `whiff_rate` ranges from 0 (no swings and misses) to 1 (all swings missed)
- `pfx_x` and `pfx_z` are gravity-adjusted movement values
- Columns for pitch types not used by a pitcher will contain null values
